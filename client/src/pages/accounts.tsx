...
export default function Accounts() {
  ...
  const updateManualPaymentMutation = useMutation({
    mutationFn: async (data: any) => {
      const res = await apiRequest("PATCH", `/api/manual-payments/${data.paymentId}`, data);
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/accounts', selectedAccount?.id, 'manual-payments'] });
      ...
      <Dialog open={showEditModal} onOpenChange={setShowEditModal}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto bg-gradient-to-br from-[#0f172a] via-[#1e293b] to-[#334155] border-white/20 text-white">
          ...
          <div className="border-t border-white/20 pt-4 mt-4">
            <Button type="button" variant="ghost" className="text-white hover:bg-white/10 w-full justify-start" onClick={() => setShowManualPayments(!showManualPayments)}>
              📋 Manual Payments {showManualPayments ? '▼' : '▶'}
            </Button>
            ...
            {showManualPayments && (
              <div className="mt-4 space-y-4">
                ...
                {Array.isArray(manualPayments) && (manualPayments as any[]).length > 0 && (
                        ...
                        <div key={payment.id} className="rounded-lg border border-white/10 bg-white/5 p-3 text-sm">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              ...
                              <span className="font-medium text-white">{(Number(payment.amountCents) / 100).toLocaleString('en-US', { style: 'currency', currency: 'USD' })}</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <Select
                                value={payment.status || 'pending'}
                                onValueChange={(value) => {
                                  updateManualPaymentMutation.mutate({ paymentId: payment.id, status: value });
                                }}
                              >
                          ...
                          {payment.notes && <p className="mt-1 text-xs text-white/50">{payment.notes}</p>}
                        </div>
                      ))}
                    <div className="flex justify-between rounded-lg border border-white/20 bg-white/10 p-2 text-sm">
