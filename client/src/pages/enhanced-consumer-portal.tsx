...
export default function EnhancedConsumerPortal() {
  ...
  const manualPaymentsUrl = encodedEmail && encodedTenantSlug ? `/api/consumer/manual-payments/${encodedEmail}${tenantQuery}` : "";
  const { data: consumerManualPayments } = useQuery<any>({
    queryKey: manualPaymentsUrl ? [manualPaymentsUrl] : ['consumer-manual-payments'],
                  ...
                  <form onSubmit={handleCallbackSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <Select value={callbackForm.requestType} onValueChange={(value) => handleInputChange("requestType", value)}>
                ...
                <Card className="border-blue-200 bg-blue-50/50">
                  <CardContent className="pt-6">
                    <div className="space-y-2">
                      {(consumerManualPayments as any[]).map((pmt: any) => (
                    ...
                    <div className="mt-3 pt-3 border-t flex justify-between text-sm">
                      ...
                      <span className="font-semibold text-emerald-700">
                        {((consumerManualPayments as any[]).filter((p: any) => p.status === 'paid').reduce((sum: number, p: any) => sum + Number(p.amountCents), 0) / 100).toLocaleString('en-US', { style: 'currency', currency: 'USD' })}
                      </span>
                    </div>
                  </CardContent>
                </Card>
