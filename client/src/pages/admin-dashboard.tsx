...
export default function AdminDashboard() {
  ...
  const { data: conversationData, isLoading: isLoadingConversation } = useQuery<{
    ...
    queryKey: ['/api/consumers', selectedAccount?.consumer?.id, 'conversation'],
    enabled: showConversationDialog && !!selectedAccount?.consumer?.id,
  });
          ...
          {selectedAccount && (
            <>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-4">
                  <span className="rounded-xl bg-purple-500/20 p-2 text-purple-300">
                    <MessageSquare className="h-5 w-5" />
                  </span>
                  <div className="flex-1">
                    <p className="text-xs uppercase tracking-wide text-blue-100/60">Message History</p>
                    <p className="text-sm font-semibold text-white">
          ...
          {selectedAccount && (
            <>
              <DialogHeader className="space-y-2 text-left pb-4 border-b border-white/10">
                ...
                <DialogDescription className="text-sm text-blue-100/70">
                  All communications with {selectedAccount.consumer?.firstName} {selectedAccount.consumer?.lastName}
                </DialogDescription>
                      ...
                      >
                        <div className={`max-w-[75%] rounded-xl p-4 ${
                          ...
                          <div className="text-sm text-blue-100/90 whitespace-pre-wrap">
                            {message.body || message.message || message.messageContent || 'No content'}
                          </div>
                  ...
                  <div className="flex items-center justify-center py-12">
                    <div className="text-center">
                      ...
                      <p className="text-xs text-blue-100/40 mt-1">
                        Start a conversation using the contact options
                      </p>
                    </div>
                  </div>
                )}
