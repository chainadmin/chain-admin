...
export function CommunicationsPanel() {
  ...
  const bucketAddonAction = useMutation({ mutationFn: ({ tenantId, step, body }: any) => apiRequest('POST', `/api/admin/communications/caller-id-bucket-addon/${tenantId}/${step}`, body), onSuccess: () => queryClient.invalidateQueries({ queryKey: ['/api/admin/communications'] }) });
  ...
  const callerIdBucketAddons = data?.callerIdBucketAddons || [];
  return <section id="admin-communications" className="mb-8 scroll-mt-4 space-y-5">
    ...
    <div className="overflow-x-auto rounded-2xl border border-white/10 bg-white/5 p-5"><h3 className="mb-4 font-semibold text-white">Company Voice routing status</h3><table className="w-full min-w-[900px] text-sm"><thead className="text-left text-blue-100/50"><tr>{['Company','Numbers','Buckets','Greeting','Hold music','Park music','Voicemail','Recording status'].map(x=><th className="pb-3 pr-4" key={x}>{x}</th>)}</tr></thead><tbody>{companies.filter((company:any)=>numbers.some((number:any)=>number.tenantId===company.id)).map((company:any)=>{const companySettings=settings.find((row:any)=>row.tenantId===company.id);const voicemail=voicemailStatus.find((row:any)=>row.tenantId===company.id);return <tr className="border-t border-white/10 text-blue-50" key={company.id}><td className="py-3 pr-4">{company.name}</td><td>{numbers.filter((number:any)=>number.tenantId===company.id).length}</td><td>{buckets.filter((bucket:any)=>bucket.tenantId===company.id).length}</td><td>{companySettings?.inboundGreetingEnabled?companySettings.inboundGreetingType:'Disabled'}</td><td>{companySettings?.holdMusicKey||'art-gallery-museum'}</td><td>{companySettings?.parkMusicKey||'art-gallery-museum'}</td><td>{voicemail?`${voicemail.unread} unread / ${voicemail.total} total`:'0'}</td><td>{voicemail?.recording?`${voicemail.recording} recording`:'Ready'}</td></tr>})}</tbody></table></div>
     ...
     <div className="overflow-x-auto rounded-2xl border border-white/10 bg-white/5 p-5"><h3 className="mb-4 font-semibold text-white">Caller ID Bucket List add-on requests</h3><table className="w-full min-w-[700px] text-sm"><thead className="text-left text-blue-100/50"><tr>{['Company','Requested by','Agreed at','Price / mo','Status','Actions'].map(x=><th className="pb-3 pr-4" key={x}>{x}</th>)}</tr></thead><tbody>{callerIdBucketAddons.map((a:any)=>{const company=companies.find((c:any)=>c.id===a.tenantId); return <tr className="border-t border-white/10 text-blue-50" key={a.tenantId}><td className="py-3 pr-4">{company?.name||a.tenantId}</td><td>{a.requestedBy}</td><td>{a.agreedAt?new Date(a.agreedAt).toLocaleString():'—'}</td><td>${((a.monthlyPriceCents||0)/100).toFixed(2)}</td><td><Badge>{a.status}</Badge>{a.status==='DENIED'&&a.deniedReason&&<p className="mt-1 text-xs text-blue-100/50">{a.deniedReason}</p>}</td><td><div className="flex gap-2">{a.status==='REQUESTED'&&<><Button size="sm" className="bg-emerald-600" onClick={()=>window.confirm(`Approve $${((a.monthlyPriceCents||0)/100).toFixed(2)}/month for ${company?.name||a.tenantId}? This adds a recurring charge to their Chiamo subscription.`)&&bucketAddonAction.mutate({tenantId:a.tenantId,step:'approve'})}>Approve</Button><Button size="sm" variant="outline" onClick={()=>{const reason=window.prompt('Reason for denial (optional):')||undefined;bucketAddonAction.mutate({tenantId:a.tenantId,step:'deny',body:{reason}});}}>Deny</Button></>}{a.status==='APPROVED'&&<Button size="sm" variant="outline" onClick={()=>window.confirm(`Revoke this add-on for ${company?.name||a.tenantId}? The recurring charge will be removed.`)&&bucketAddonAction.mutate({tenantId:a.tenantId,step:'revoke'})}>Revoke</Button>}</div></td></tr>})}</tbody></table>{!callerIdBucketAddons.length&&<p className="py-8 text-center text-sm text-blue-100/60">No caller ID bucket list requests.</p>}</div>
