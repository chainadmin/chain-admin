...
import { PrivacyLineSettings } from "@/components/voip/PrivacyLineSettings";
import { GreetingAudioInput } from "@/components/voip/GreetingAudioInput";
...
type VoiceNumber = { id: string; phoneNumber: string; friendlyName?: string; bucketId?: string | null; routingBucketId?: string | null; numberType?: string; areaCode?: string | null; state?: string | null; localPresenceCallerIdEnabled?: boolean | null };
...
type CallerIdBucketAddon = { status: "REQUESTED" | "APPROVED" | "DENIED" | "CANCELLED"; monthlyPriceCents: number; deniedReason?: string | null } | null;
...
export function VoipControlCenter({ tone = "chain", initialSection = "routing" }: { tone?: "chain" | "chiamo"; initialSection?: "routing" | "experience" | "voicemail" }) {
  ...
  const addon = useQuery<CallerIdBucketAddon>({ queryKey: key("/api/voip/caller-id-bucket-addon") });
  ...
  const toggleCallerIdBucket = useMutation({ mutationFn: ({ id, enabled }: { id: string; enabled: boolean }) => apiRequest("PATCH", `/api/voip/phone-numbers/${id}`, { localPresenceCallerIdEnabled: enabled }), onSuccess: refresh });
  const requestAddon = useMutation({ mutationFn: () => apiRequest("POST", "/api/voip/caller-id-bucket-addon/request", { agreed: true }), onSuccess: () => qc.invalidateQueries({ queryKey: key("/api/voip/caller-id-bucket-addon") }) });
  const setBucketRouting = useMutation({ mutationFn: (routeToVoicemail: boolean) => apiRequest("PATCH", "/api/voip/settings/caller-id-bucket-routing", { routeToVoicemail }), onSuccess: () => qc.invalidateQueries({ queryKey: key("/api/voip/settings") }) });
  ...
  return <div className="space-y-5">
    ...
    {section === "routing" && <Routing buckets={buckets.data || []} users={(users.data || []).filter(user => user.isActive !== false && user.voipAccess === true)} numbers={numbers.data || []} accent={accent} dark={tone === "chain"} assign={assign.mutate} remove={deleteBucket.mutate} editing={editing} setEditing={setEditing} creating={creating} setCreating={setCreating} toggleCallerIdBucket={toggleCallerIdBucket.mutate} addon={addon.data ?? null} addonLoading={addon.isLoading} requestAddon={() => requestAddon.mutate()} requestingAddon={requestAddon.isPending} routesToVoicemail={settings.data?.callerIdBucketRoutesToVoicemail ?? false} setRoutesToVoicemail={setBucketRouting.mutate} settingRoutesToVoicemail={setBucketRouting.isPending} />}
...
function Routing({ buckets, users, numbers, accent, dark, assign, remove, editing, setEditing, creating, setCreating, toggleCallerIdBucket, addon, addonLoading, requestAddon, requestingAddon, routesToVoicemail, setRoutesToVoicemail, settingRoutesToVoicemail }: { buckets: Bucket[]; users: TeamMember[]; numbers: VoiceNumber[]; accent: string; dark: boolean; assign: (x: {id:string;bucketId:string|null}) => void; remove:(id:string)=>void; editing:Bucket|null; setEditing:(x:Bucket|null)=>void; creating:boolean; setCreating:(x:boolean)=>void; toggleCallerIdBucket: (x: { id: string; enabled: boolean }) => void; addon: CallerIdBucketAddon; addonLoading: boolean; requestAddon: () => void; requestingAddon: boolean; routesToVoicemail: boolean; setRoutesToVoicemail: (v: boolean) => void; settingRoutesToVoicemail: boolean }) {
  const panel = dark ? "border-white/10 bg-white/5 text-white" : "border-slate-200 bg-white";
  const localNumbers = numbers.filter(n => n.numberType === "LOCAL_PRESENCE");
  ...
  <section className={`rounded-2xl border p-5 xl:col-span-2 ${panel}`}>
    <h3 className="font-bold">Caller ID bucket list add-on</h3>
    <p className={dark ? "mt-1 max-w-2xl text-sm text-blue-100/60" : "mt-1 max-w-2xl text-sm text-slate-500"}>Add a local number here to show it automatically as your caller ID when calling that area — a whole state, if that's all you've bought for it (an area code covers itself; add one number per state for full coverage).</p>
    <div className="mt-5">
      {addonLoading ? <div className="h-24 animate-pulse rounded-xl bg-slate-200/40" /> : <CallerIdBucketAddonGate dark={dark} accent={accent} addon={addon} requestAddon={requestAddon} requestingAddon={requestingAddon} />}
    </div>
    {addon?.status === "APPROVED" && <div className="mt-5 space-y-4">
      ...
      <div className="space-y-2">
        {localNumbers.map(n => <div className={`flex flex-wrap items-center justify-between gap-3 rounded-xl border p-3 ${dark ? "border-white/10 bg-slate-950/20" : "border-slate-100 bg-slate-50"}`} key={n.id}>
          <div><p className="font-mono font-semibold">{n.phoneNumber}</p><p className="text-xs opacity-60">{[n.areaCode ? `Area code ${n.areaCode}` : null, n.state].filter(Boolean).join(" · ") || n.friendlyName || "Local number"}</p></div>
          <label className="flex items-center gap-2 text-sm font-semibold"><span className={n.localPresenceCallerIdEnabled ? (dark ? "text-emerald-300" : "text-emerald-700") : "opacity-60"}>{n.localPresenceCallerIdEnabled ? "In bucket list" : "Not in bucket list"}</span><Switch checked={!!n.localPresenceCallerIdEnabled} onCheckedChange={v => toggleCallerIdBucket({ id: n.id, enabled: v })} /></label>
        </div>)}
        {!localNumbers.length && <p className="py-8 text-center text-sm opacity-60">Buy a local-area-code number from Numbers to start a bucket list.</p>}
      </div>
    </div>}
  </section>
...
function CallerIdBucketAddonGate({ dark, accent, addon, requestAddon, requestingAddon }: { dark: boolean; accent: string; addon: CallerIdBucketAddon; requestAddon: () => void; requestingAddon: boolean }) {
  const [agreed, setAgreed] = useState(false);
  const priceLabel = `$${(((addon?.monthlyPriceCents) ?? 4000) / 100).toFixed(2)}/month`;
  if (addon?.status === "APPROVED") {
    return <p className={`rounded-xl border px-3 py-2.5 text-sm ${dark ? "border-emerald-400/30 bg-emerald-500/10 text-emerald-200" : "border-emerald-200 bg-emerald-50 text-emerald-800"}`}>Approved — billing {priceLabel}. Add specific numbers to the bucket list below.</p>;
  }
  if (addon?.status === "REQUESTED") {
    return <p className={`rounded-xl border px-3 py-2.5 text-sm ${dark ? "border-amber-400/30 bg-amber-500/10 text-amber-200" : "border-amber-200 bg-amber-50 text-amber-800"}`}>Requested — awaiting Global Admin approval before it can be billed and used.</p>;
  }
  return <div className={`rounded-xl border p-4 ${dark ? "border-white/10 bg-slate-950/20" : "border-slate-100 bg-slate-50"}`}>
    {addon?.status === "DENIED" && <p className={`mb-3 text-sm ${dark ? "text-rose-300" : "text-rose-600"}`}>Your previous request was denied{addon.deniedReason ? `: ${addon.deniedReason}` : "."} You can request again below.</p>}
    {addon?.status === "CANCELLED" && <p className={`mb-3 text-sm ${dark ? "text-blue-100/60" : "text-slate-500"}`}>This add-on was previously cancelled. You can request it again below.</p>}
    <p className="font-semibold">This is a paid add-on: {priceLabel}</p>
    <p className={dark ? "mt-1 text-sm text-blue-100/60" : "mt-1 text-sm text-slate-500"}>Turning it on submits a request to Global Admin. Once approved, {priceLabel.replace("/month", "")}/month is added to your Chiamo bill and you can start adding numbers to your bucket list.</p>
    <label className="mt-4 flex items-start gap-2 text-sm"><input type="checkbox" className="mt-0.5 h-4 w-4" checked={agreed} onChange={e => setAgreed(e.target.checked)} />I agree to the {priceLabel} recurring charge for this feature.</label>
    <Button className={`mt-3 ${accent}`} disabled={!agreed || requestingAddon} onClick={requestAddon}>{requestingAddon ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}Request activation</Button>
  </div>;
}
...
function VoicemailPlayer({message,onHeard}:{message:VoiceMail;onHeard:()=>void}) {
  ...
  async function load(){
    ...
    try {
      const response=await apiRequest("GET",`/api/voip/voicemail/${message.id}/listen`);
      const {url:mediaUrl}=await response.json() as {url:string};
      const media=await fetch(mediaUrl,{cache:"no-store",credentials:"same-origin"});
      if(!media.ok)throw new Error(media.status===404?"Voicemail audio is not ready yet.":"Voicemail audio is unavailable.");
      setUrl(URL.createObjectURL(await media.blob()));
      onHeard();
    } catch(error) {
      setError(error instanceof Error?error.message:"Voicemail audio is unavailable.");
    } finally { setLoading(false); }
  }
  if(url)return <audio className="h-10 w-full min-w-56 max-w-sm" controls preload="metadata" src={url}/>;
  return <div><Button type="button" size="sm" variant="outline" disabled={loading} onClick={load}><Play className="mr-2 h-4 w-4"/>{loading?"Loading…":"Listen"}</Button>{error&&<p className="mt-1 text-xs text-rose-600">{error}</p>}</div>;
}

function Inbox({mail,dark,update,remove}:{mail:VoiceMail[];dark:boolean;update:(x:{id:string;read:boolean})=>void;remove:(id:string)=>void}){const panel=dark?"border-white/10 bg-white/5 text-white":"border-slate-200 bg-white";const Message=({m}:{m:VoiceMail})=><article className={`flex flex-wrap items-center gap-4 border-b p-5 last:border-0 ${!m.isRead ? dark?"bg-sky-400/10":"bg-emerald-50/60" : ""}`}><VoicemailPlayer message={m} onHeard={()=>{if(!m.isRead)update({id:m.id,read:true})}}/><div className="min-w-[180px] flex-1"><p className="font-mono font-semibold">{m.fromNumber}</p><p className="mt-1 text-xs opacity-60">{new Date(m.createdAt).toLocaleString()} · {duration(m.duration)}</p></div><div className="flex gap-1"><Button size="sm" variant="outline" onClick={()=>update({id:m.id,read:!m.isRead})}>{m.isRead?"Mark unheard":"Mark heard"}</Button><Button size="icon" variant="ghost" onClick={()=>remove(m.id)} aria-label="Delete voicemail"><Trash2 className="h-4 w-4 text-rose-500"/></Button></div></article>;const normal=mail.filter(m=>!m.isPrivacy);const privacy=mail.filter(m=>m.isPrivacy);const Empty=({privacyOnly}:{privacyOnly?:boolean})=><div className="py-10 text-center"><Volume2 className="mx-auto h-8 w-8 opacity-30"/><p className="mt-2 font-semibold">No {privacyOnly?"Privacy Line ":""}voicemails</p><p className="mt-1 text-sm opacity-60">{privacyOnly?"Callbacks to the dedicated Privacy Line appear here.":"New caller messages will appear here."}</p></div>;return <div className="space-y-5"><section className={`rounded-2xl border ${panel}`}><div className="border-b p-5"><h3 className="font-bold">Voicemail inbox</h3><p className={dark?"mt-1 text-sm text-blue-100/60":"mt-1 text-sm text-slate-500"}>Messages left on your regular business numbers.</p></div>{normal.map(m=><Message key={m.id} m={m}/>)}{!normal.length&&<Empty/>}</section><section className={`rounded-2xl border ${panel}`}><div className="border-b p-5"><h3 className="font-bold">Privacy Voicemail</h3><p className={dark?"mt-1 text-sm text-blue-100/60":"mt-1 text-sm text-slate-500"}>Messages left after a callback to the dedicated Privacy Line.</p></div>{privacy.map(m=><Message key={m.id} m={m}/>)}{!privacy.length&&<Empty privacyOnly/>}</section></div>}
