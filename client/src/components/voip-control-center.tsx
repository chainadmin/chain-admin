...
function Routing({ buckets, users, numbers, accent, dark, assign, remove, editing, setEditing, creating, setCreating, toggleCallerIdBucket, addon, addonLoading, requestAddon, requestingAddon }: { buckets: Bucket[]; users: TeamMember[]; numbers: VoiceNumber[]; accent: string; dark: boolean; assign: (x: {id:string;bucketId:string|null}) => void; remove:(id:string)=>void; editing:Bucket|null; setEditing:(x:Bucket|null)=>void; creating:boolean; setCreating:(x:boolean)=>void; toggleCallerIdBucket: (x: { id: string; enabled: boolean }) => void; addon: CallerIdBucketAddon; addonLoading: boolean; requestAddon: () => void; requestingAddon: boolean }) {
  ...
  <section className={`rounded-2xl border p-5 xl:col-span-2 ${panel}`}>
    ...
    {addon?.status === "APPROVED" && <div className="mt-5 space-y-2">
