/**
 * Static NANP (North American Numbering Plan) area-code-to-state table for US
 * area codes that belong to exactly one state. This lets a single purchased
 * Local Presence ("bucket") number automatically cover every area code in its
 * state — e.g. one NY number covers 212, 718, 585, 716, etc. — without
 * requiring enrollment in the separate, Global-Admin-approved bulk Local
 * Presence package system (`localPresencePackages`), which remains the source
 * of truth when it has an explicit mapping for a given area code.
 *
 * Only area codes that map unambiguously to a single US state are listed.
 * Area codes shared across multiple states/provinces (e.g. many overlay
 * codes), and non-US NANP members, are intentionally omitted — for those, an
 * exact area-code match is still used automatically, they just don't expand
 * to a whole-state match.
 */
export const NANP_AREA_CODE_STATE: Readonly<Record<string, string>> = Object.freeze({
  // Alabama
...
export function resolveNanpAreaCodeState(areaCode: string): string | undefined {
  return NANP_AREA_CODE_STATE[areaCode];
}
