...
function harness(database:any, limiter = new ChiamoUserReauthLimiter()) {
  ...
  const app:any={};
  for(const method of ["get","post","patch"]) app[method]=(path:string,...handlers:any[])=>{registered[`${method} ${path}`]=handlers};
  ...
  return {
    request:async(method:string,path:string,user:any,body:any={})=>{
      ...
      const req:any={user,body,params:{id:path.split("/").at(-1)},ip:"127.0.0.1",socket:{}};
...
const owner={tenantId:"11111111-1111-4111-8111-111111111111",id:"aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",role:"owner",product:"chiamo"};
const member={...owner,id:"bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb",role:"agent"};
const targetId="cccccccc-cccc-4ccc-8ccc-cccccccccccc";
const tenant={id:owner.tenantId,chiamoConnectEnabled:true,chainCoreEnabled:false,maxActiveUsers:3,businessType:"business"};
...
test("HTTP inputs cannot promote a managed member to owner",async()=>{
  const database=new QueueDatabase();
  const create=await harness(database).request("POST","/api/chiamo/team-members",owner,{username:"newagent",email:"new@example.com",role:"owner",ownerPassword:"anything"});
...
test("HTTP create accepts client empty optional names and issues exact temporary-password fields",async()=>{
  const passwordHash=await bcrypt.hash("Owner-current-password!2",4);
  const inserted={...target,firstName:null,lastName:null,mustChangePassword:true,temporaryPasswordExpiresAt:new Date("2026-01-03T00:00:00.000Z"),createdAt:new Date(),updatedAt:new Date(),lastLoginAt:null};
  const database=new QueueDatabase([[tenant],[{...ownerRow,passwordHash}],[ownerRow],[{planId:"starter",billingStatus:"ACTIVE"}],[],[inserted]]);
  const res=await harness(database).request("POST","/api/chiamo/team-members",owner,{username:" New.Agent ",email:" NEW@Example.COM ",firstName:"",lastName:"",role:"agent",voipAccess:true,ownerPassword:"Owner-current-password!2"});
  assert.equal(res.statusCode,201);
  assert.equal(res.body.credentials.username,"agent");
  assert.equal(typeof res.body.credentials.temporaryPassword,"string");
  assert.ok(res.body.credentials.temporaryPassword.length>=12);
  assert.equal(res.body.credentials.expiresAt,"2026-01-03T00:00:00.000Z");
  ...
  assert.equal(values.temporaryPasswordExpiresAt.toISOString(),"2026-01-03T00:00:00.000Z");
  assert.deepEqual(values.restrictedServices,["billing"]);
  assert.notEqual(values.passwordHash,res.body.credentials.temporaryPassword);
...
test("wrong owner password is rate limited and never writes credentials",async()=>{
  ...
  const body={username:"newagent",email:"new@example.com",ownerPassword:"wrong"};
...
test("deactivation increments credential version to revoke existing sessions",async()=>{
  ...
  const database=new QueueDatabase([[tenant],[target],[ownerRow,target],[],[updated]]);
...
test("activation over the locked active-seat cap is rejected without a write",async()=>{
  ...
  const database=new QueueDatabase([[cappedTenant],[inactive],[ownerRow,active2]]);
...
test("billable-seat increases require HTTP confirmation before insertion",async()=>{
  ...
  const res=await harness(database).request("POST","/api/chiamo/team-members",owner,{username:"newagent",email:"new@example.com",ownerPassword:"Owner-current-password!2"});
  assert.equal(res.statusCode,409);
...
test("adding or reactivating requires active billing, while deactivation remains safe",async()=>{
  ...
  const create=await harness(createDb).request("POST","/api/chiamo/team-members",owner,{username:"newagent",email:"new@example.com",ownerPassword:"Owner-current-password!2"});
  ...
  const activateDb=new QueueDatabase([[tenant],[inactiveTarget],[ownerRow],[inactiveSubscription]]);
...
test("serialized concurrent creates cannot exceed the seat cap",async()=>{
  ...
  const database:any={
    transaction(fn:(tx:any)=>Promise<any>){
      const run=lock.then(async()=>{
        ...
        const inserted={...target,mustChangePassword:true,temporaryPasswordExpiresAt:new Date("2026-01-03"),createdAt:new Date(),updatedAt:new Date(),lastLoginAt:null};
  ...
  const body={username:"newagent",email:"new@example.com",ownerPassword:"Owner-current-password!2",confirmSeatPriceImpact:true};
