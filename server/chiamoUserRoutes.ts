...
import { generateTemporaryPassword, TEMPORARY_PASSWORD_TTL_MS } from "./chiamoCredentialAuth";
...
const createInput = z.object({
  ...
  voipAccess: z.boolean().default(false),
  // Password verification is deliberately required only when credentials are issued.
...
export function registerChiamoUserRoutes(app: Express, dependencies: ChiamoUserRoutesDependencies = {}) {
  ...
  const tenantForUpdate = async (tx: any, tenantId: string) => {
    const [tenant] = await tx.select({
      id: tenants.id, chiamoConnectEnabled: tenants.chiamoConnectEnabled, chainCoreEnabled: tenants.chainCoreEnabled,
  ...
  const requireChiamoTenant = (tenant: any) => tenant?.chiamoConnectEnabled === true && tenant?.chainCoreEnabled === false;
  ...
  app.get("/api/chiamo/team-members", authenticate, route(async (req, res) => {
    ...
    const [tenantRows, members, subscription] = await Promise.all([
      database.select({ id: tenants.id, chiamoConnectEnabled: tenants.chiamoConnectEnabled, chainCoreEnabled: tenants.chainCoreEnabled, maxActiveUsers: tenants.maxActiveUsers, businessType: tenants.businessType }).from(tenants).where(eq(tenants.id, req.user.tenantId)).limit(1),
  ...
  app.post("/api/chiamo/team-members", authenticate, route(async (req, res) => {
    ...
    const created = await database.transaction(async (tx: any) => {
      ...
      const [actor] = await tx.select().from(agencyCredentials).where(and(eq(agencyCredentials.id, req.user.id), eq(agencyCredentials.tenantId, tenant.id), eq(agencyCredentials.role, "owner"), eq(agencyCredentials.isActive, true))).limit(1).for("update");
      ...
      const duplicate = await tx.select({ id: agencyCredentials.id }).from(agencyCredentials)
        .where(sql`lower(trim(${agencyCredentials.username})) = ${normalized(input.username)} OR lower(trim(${agencyCredentials.email})) = ${normalized(input.email)}`).limit(1).for("update");
      ...
      const [member] = await tx.insert(agencyCredentials).values({
        tenantId: tenant.id, username: normalized(input.username), email: normalized(input.email),
        ...
        passwordHash: await bcrypt.hash(temporaryPassword, 12), mustChangePassword: true,
        temporaryPasswordExpiresAt: expiresAt, credentialVersion: 1, updatedAt: now(),
      }).returning();
      return { status: 201, body: { member: safeMember(member), credentials: { username: member.username, temporaryPassword, expiresAt: expiresAt.toISOString() }, seats: { ...seat, ...chiamoUserPriceImpact(subscription, seat.activeUsers + 1) } } };
    });
  ...
  app.patch("/api/chiamo/team-members/:id", authenticate, route(async (req, res) => {
    ...
    const result = await database.transaction(async (tx: any) => {
      ...
      for (const key of ["email", "firstName", "lastName", "role", "voipAccess", "isActive"] as const) if (input[key] !== undefined) changes[key] = key === "email" ? normalized(input[key] as string) : input[key];
      if (changes.email) {
