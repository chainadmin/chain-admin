...
- **Debt Manager Pro Integration**: Standalone API integration for DMP collection software, completely separate from SMAX. Features bearer token authentication, manual account import (no automatic syncing), inline payment posting after Chain payments, SMS/email communication logging, and notes syncing. DMP is guarded by `dmpEnabled` flag in tenant settings with configurable API URL, username, and password. Settings UI includes enable toggle, credential fields, connection test button, and manual import accounts button. DMP service supports: portfolios, accounts, payments, notes, attempts, phone management, and softphone/VoIP integration hooks.
...
- **Unified Communications**: Merges email and SMS functionalities with templates, campaigns, callback requests, and an automation processor. Features real-time SMS campaign tracking, multi-number SMS sending, campaign resume from interruption points (`lastSentIndex`), and a comprehensive variable replacement system. Includes Scheduled, Sequence, and Event-based automations. Email templates utilize a WYSIWYG editor with secure HTML preview.
- **SMS Compliance (TCPA)**: Comprehensive opt-out handling with STOP/STOPALL/UNSUBSCRIBE keyword detection via inbound SMS webhook. Consumers who reply STOP are marked `smsOptedOut=true` and added to tenant-specific `sms_blocked_numbers` table. Twilio status callbacks track permanent delivery failures (error codes 21211, 21610, 21614, 21408, 30003, 30005, 30006, 30007) and automatically block undeliverable numbers. Pre-send guards in `smsService.ts` check opt-out status and blocked numbers before any SMS transmission. Campaign audience resolution filters exclude opted-out consumers and blocked phone numbers. Features a Compliance tab in the SMS page with historical Twilio data sync (scan message history for past STOP replies and delivery failures) and blocked numbers management UI.
...
- **Account Management**: Enhanced folder organization and CSV import integration with an optional status column. Features fully customizable blocked account statuses defined by tenants, preventing communications and payments, with case-insensitive validation across all payment paths and communication filters. CSV import extracts SSN Last 4 digits from socialsecuritynumber column and stores in consumer.ssnLast4, accessible via {{ssnLast4}} template variable for secure consumer identification.
